...
This is an analytical and calibration tool — not a trading system. Full specification: `docs/covered call simulation requirements.docx`.
...
- **Monitors** daily closes and **rolls** the contract forward when the price approaches the strike, collecting or paying the difference.
...
| `--roll-trigger-pct` | 2.0 | Roll when price is within this % of the strike |
...
### Example: three tickers with default parameters
...
One row per contract showing: contract number, open/close dates, days held, stock price and strike at open, HV20 volatility, theoretical and received premium, close reason, stock price and cost-to-close at close, new premium (if rolled), net P&L, running cash balance, reserve remaining, and an HV20 warning flag.
